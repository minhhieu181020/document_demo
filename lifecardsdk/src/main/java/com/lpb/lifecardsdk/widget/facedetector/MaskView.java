...
public class MaskView extends ConstraintLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        View view = LayoutInflater.from(context).inflate(R.layout.lifecardsdk_layout_mask_view, this, true);
        this.borderView = view.findViewById(R.id.iv_id_card_mask_background);
        this.maskView = view.findViewById(R.id.camera_mask);
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.LifeCardSDKMaskView, 0, 0);
        this.percentWidth = a.getFloat(R.styleable.LifeCardSDKMaskView_percentWidth, 1.0F);
        this.percentHeight = a.getFloat(R.styleable.LifeCardSDKMaskView_percentHeight, 1.0F);
        this.backgroundSrc = a.getResourceId(R.styleable.LifeCardSDKMaskView_backgroundSrc, 0);
        this.backgroundInnerView = a.getResourceId(R.styleable.LifeCardSDKMaskView_backgroundInnerView, 0);
        this.ratioWidth = a.getInt(R.styleable.LifeCardSDKMaskView_ratioWidth, 1);
        this.ratioHeight = a.getInt(R.styleable.LifeCardSDKMaskView_ratioHeight, 1);
    }

    @SuppressLint({"DefaultLocale"})
    public void updateConstraint(MaskView.ViewMode viewMode, ConstraintLayout parentView) {
        ConstraintSet constraintSet = new ConstraintSet();
        constraintSet.clone(this);
        ConstraintSet rootSet = new ConstraintSet();
        rootSet.clone(parentView);
        if (viewMode == MaskView.ViewMode.LANDSCAPE) {
