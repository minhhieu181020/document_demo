...
public class LivenessDetector {
    ...
    public void setFaceActions(List<String> faceActions, int numberAction) {
        this.faceActions.clear();
        this.faceActions.addAll(faceActions);
        setAllSteps(numberAction);
    }

    private void setAllSteps(int numberAction) {
        ...
        if (faceActions != null && faceActions.size() > 0) {
            List<String> randomFaceActions = randomFaceAction(faceActions, numberAction);
            if (randomFaceActions.size() > 0) {
                for (String faceAction : randomFaceActions) {
    ...
    private List<String> randomFaceAction(List<String> faceActions, int numberAction) {
        List<String> randomFaceActions = new ArrayList<>();
        int size = faceActions.size();
        if (size > numberAction) {
        ...
        if (!this.isDetecting && this.readyForNextStep) {
            ...
            this.mDetector.detectInImage(vsImage).addOnSuccessListener(new OnSuccessListener<List<FirebaseVisionFace>>() {
                ...
                public void onSuccess(List<FirebaseVisionFace> allFaces) {
                    ...
                    if (faces.isEmpty()) {
                        LivenessDetector.this.showErrorMessage(LivenessDetector.this.mContext.getString(R.string.lifecardsdk_face_not_found));
                        ...
                        LivenessDetector.this.showErrorMessage(LivenessDetector.this.mContext.getString(R.string.lifecardsdk_face_multiple));
                        if (LivenessDetector.this.mCurrentStepIndex != -1) {
                            if (LivenessDetector.this.mDetectorListener != null) {
                                LivenessDetector.this.mDetectorListener.onResetStep();
                            }
                        ...
                        if (!LivenessDetector.this.isStarted) {
                            ...
                            if (faceTooSmall) {
                                LivenessDetector.this.showErrorMessage(LivenessDetector.this.mContext.getString(R.string.lifecardsdk_face_too_small));
                                return;
                            }

                            if (faceTooLarge) {
                                LivenessDetector.this.showErrorMessage(LivenessDetector.this.mContext.getString(R.string.lifecardsdk_face_too_large));
                                return;
                            }

                            if (!faceIsCenter) {
                                LivenessDetector.this.showErrorMessage(LivenessDetector.this.mContext.getString(R.string.lifecardsdk_face_out_of_rect));
                                return;
                            }
                        }
